...
function History() {
  const { tradingStrategy } = useAuthStore()
  ...
  const load = useCallback(async () => {
    ...
    setErr(null)
    try {
      const r = await fetch(`${INTEGRATION_BASE}/api/trading/history?limit=1000`)
      ...
      {payload && (
        <div style={{
          ...
          <span style={{ padding: '8px 14px', background: 'rgba(255,255,255,0.06)', borderRadius: '8px' }}>
            ...
            {payload.active_strategy ? ` (${String(payload.active_strategy).toUpperCase()})` : ''}
          </span>
